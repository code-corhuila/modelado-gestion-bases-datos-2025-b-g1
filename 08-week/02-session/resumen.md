...
-- Se requiere sabes basado en el nombre del producto, la información de la categoría.
-- Mostrar nombre producto, descripción, nombre categoría, descripción.alter
SELECT 
	* 
FROM entidad_1 alistas_1
INNER JOIN entidad_2 alista_2 ON alias_1.id = alistas_2.alisas_1_id;

SELECT 
	c.name categoria,
    c.description categoria_descripcion,
    p.name producto,
    p.description prodcuto_descripcion
FROM category c
	INNER JOIN product p ON c.id = p.category_id
WHERE p.name = 'Lettuce';

-- Se requiere identificar los prodcutos que estan inactivos. Muestre el nombre de la categoría, 
-- nombre del producto, valor unitario y el estado del producto
...
-- además que sea superiro a 4 el valor unitario
SELECT 
	c.name categoria,    
    p.name producto,
    p.id,
